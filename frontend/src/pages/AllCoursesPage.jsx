...
export const AllCoursesPage = () => {
  ...
  const fetchAllCourses = useCallback(async (page = 1) => {
    try {
      setLoading(true);
      const cacheKey = `all:courses:page:${page}`;
  ...
  const fetchEnrolledCourses = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/enrollments/my-courses`, {
        params: { per_page: 1000 } // Get all enrolled courses
      });

      if (response.data.success) {
        const enrolledIds = new Set(response.data.data.map(course => course.id));
  ...
  useEffect(() => {
    // Fetch enrolled courses on mount (this must complete first)
    fetchEnrolledCourses();
  }, [fetchEnrolledCourses]);

  useEffect(() => {
    // Only fetch all courses after enrolled courses are loaded
    if (!enrolledLoading) {
      fetchAllCourses(currentPage);
    }
  }, [currentPage, enrolledLoading, fetchAllCourses]);

  // Filter courses based on search query and enrolled status
  useEffect(() => {
    let filtered = allCourses.filter(course => !enrolledCourseIds.has(course.id));
  ...
  }, [allCourses, enrolledCourseIds, searchQuery]);

  const handleCourseClick = (courseId) => {
    navigate(`/student-course/${courseId}`, { state: { isEnrolled: false } });
  };
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-12">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-4xl font-bold text-slate-900">Explore All Courses</h1>
            <button
              onClick={() => navigate('/student-dashboard')}
              className="flex items-center gap-2 px-6 py-3 bg-slate-200 text-slate-900 rounded-lg font-medium hover:bg-slate-300 transition-colors"
            >
          ...
          <p className="text-lg text-slate-600 mb-8">
            Discover and enroll in {totalCourses - enrolledCourseIds.size} available course{totalCourses - enrolledCourseIds.size !== 1 ? 's' : ''}
          </p>
        ...
        {/* Loading State */}
        {(loading || enrolledLoading) && allCourses.length === 0 && (
          <div className="flex items-center justify-center min-h-64">
            <div className="text-center">
              <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-sky-100 mb-4 animate-spin">
                <div className="w-8 h-8 border-3 border-sky-200 border-t-sky-500 rounded-full"></div>
              </div>
        ...
        {!loading && filteredCourses.length === 0 && !searchQuery && (
          <div className="text-center py-16">
            ...
            <p className="text-slate-600">You are already enrolled in all available courses or there are no courses yet.</p>
          </div>
        )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
              {filteredCourses.map((course) => (
                <div
                  ...
                  className="group bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden cursor-pointer hover:scale-105 transform fade-in"
                >
                  {/* Card Header */}
                  <div className="bg-gradient-to-r from-sky-500 to-blue-600 h-32 relative overflow-hidden">
                  ...
                  <div className="px-6 py-4 bg-slate-50 border-t border-slate-100">
                    <button
                      ...
                      className="w-full px-4 py-2 bg-gradient-to-r from-sky-500 to-blue-600 text-white rounded-lg font-medium hover:shadow-lg transition-shadow"
                    >
                      View Course
                    </button>
                  </div>
