  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 fade-in">
      <div className="bg-white/95 backdrop-filter backdrop-blur-xl rounded-2xl shadow-2xl max-w-2xl w-full mx-auto max-h-[90vh] overflow-y-auto border border-slate-200">
        {/* Header */}
        <div className="sticky top-0 bg-gradient-to-r from-blue-50 to-cyan-50 border-b border-slate-200 px-4 sm:px-6 md:px-8 py-4 sm:py-6 flex justify-between items-start sm:items-center gap-4">
          <div className="flex-1 min-w-0">
            <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 break-words">Edit Lesson</h2>
            <p className="text-slate-600 text-xs sm:text-sm mt-1 hidden sm:block">Update lesson details and content</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-4 sm:px-6 md:px-8 py-6 sm:py-8">
          ...
          {/* Lesson Type Badge */}
          <div className="mb-8 bg-gradient-to-r from-blue-50 to-cyan-50 p-4 sm:p-6 rounded-xl border border-slate-200">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
              <div className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-white rounded-lg border border-slate-200">
                <span className="text-xl sm:text-2xl flex-shrink-0">{lesson.type === 'video' ? '🎥' : '📄'}</span>
                <span className="text-sm sm:text-base font-medium text-slate-900 capitalize">{lesson.type}</span>
              </div>
              <p className="text-xs sm:text-sm text-slate-600">(cannot be changed)</p>
            </div>
          </div>
